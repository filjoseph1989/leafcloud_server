...
def evaluate_classification(y_true_idx, clf_preds, export_dir):
    ...
    plt.title("Confusion Matrix - Nutrient Classification (V11)", fontsize=14, pad=15)
    plt.ylabel("Actual Label", fontsize=12)
    plt.xlabel("Predicted Label", fontsize=12)
    plt.tight_layout()

    os.makedirs(export_dir, exist_ok=True)
    save_path = os.path.join(export_dir, "confusion_matrix_v11.png")
...
def main():
    ...
    if args:
        ...
        import glob
        keras_files = sorted(glob.glob('leafcloud_multimodal_v11_*.keras'), reverse=True)
        if not keras_files:
            keras_files = sorted(glob.glob('leafcloud_multimodal_v10_*.keras'), reverse=True)
        if not keras_files:
