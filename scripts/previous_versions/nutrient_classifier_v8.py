...
from tensorflow.keras.layers import (
    ...
    Input, Concatenate, BatchNormalization, Lambda
)
...
# ==========================================
# CONFIGURATION - MULTI-TASK V8
# ==========================================
DB_URL = settings.DATABASE_URL
IMG_SIZE = (224, 224)
BATCH_SIZE = 32
TIMESTAMP = datetime.now().strftime('%Y%m%d_%H%M')
MODEL_SAVE_PATH = f'leafcloud_multimodal_v8_{TIMESTAMP}.keras'
...
def build_model_v8():
    ...
    # Batch Normalization normalizes the unbounded ReLU activations to a standard scale (~[-2, 2])
    merged_normalized = BatchNormalization()(merged)
    merged_dropout = Dropout(0.3)(merged_normalized)

    # Classification Branch
    clf_branch = Dense(64, activation='relu')(merged_dropout)
    clf_branch = Dropout(0.2)(clf_branch)
    clf_output = Dense(4, activation='softmax', name='clf_output')(clf_branch)

    # Gradient Blocking Layer
    # Prevents regression loss gradients from backpropagating into and corrupting the classification weights
    clf_output_stopped = Lambda(lambda val: tf.stop_gradient(val))(clf_output)
...
if __name__ == '__main__':
    ...
    if train_df.empty:
        ...
        print('🧠 Building Multi-Task Multi-Modal Model (V8)...')
        model, base_model = build_model_v8()
        model.summary()
        ...
        # Monitors joint loss instead of accuracy to allow the regression branch to fully converge
