...
def run_evaluation(model, val_df):
    ...
    # Keras multi-output models return a list of outputs
    # Let's map them robustly according to output names
...
def evaluate_classification(y_true_idx, clf_preds, export_dir="exports"):
    ...
    plt.title("Confusion Matrix - Nutrient Classification", fontsize=14, pad=15)
    ...
    save_path = os.path.join(export_dir, "confusion_matrix_v2.png")
...
def main():
    ...
    default_model = "leafcloud_multimodal_v2_20260520_0056.keras"
    model_path = sys.argv[1] if len(sys.argv) > 1 else default_model
    
    if not os.path.exists(model_path):
        # Fall back to searching for latest model in directory matching pattern
        import glob
        keras_files = glob.glob("leafcloud_multimodal_v2_*.keras")
        if keras_files:
