...
# ==========================================
# CONFIGURATION - MULTI-TASK V5
# ==========================================
DB_URL = settings.DATABASE_URL
IMG_SIZE = (224, 224)
BATCH_SIZE = 32
TIMESTAMP = datetime.now().strftime('%Y%m%d_%H%M')
MODEL_SAVE_PATH = f'leafcloud_multimodal_v5_{TIMESTAMP}.keras'

SENSOR_NORM = {
    'ph':         (3.0, 10.0),
    'ec':         (0.0, 1.6),   # actual data range is 0.04–1.51; (0,3) was 2x too wide
...
# [0, 1] scale + sigmoid output — same as V4, correct design
CONCENTRATION_MAP = {
    'Water': [0.0, 0.0],
    'NPK':   [1.0, 0.0],
    'Micro': [0.0, 1.0],
...
def build_model_v5():
    ...
    # Sensor Branch — EC is the primary NPK/Water differentiator
    ...
    # Dedicated Regression Branch — sigmoid bounds output to [0, 1]
...
if __name__ == '__main__':
    ...
    if train_df.empty:
        ...
        print('🧠 Building Multi-Task Multi-Modal Model (V5)...')
        model, base_model = build_model_v5()
        model.summary()
        ...
        # Establishes a strong clf baseline before any regression
        # gradient can interfere with the shared representation.
        # --------------------------------------------------
        ...
        # --------------------------------------------------
        # Phase 2: Joint training — low reg_weight preserves
        # classification accuracy (root fix vs V4's 0.3→0.8).
        # --------------------------------------------------
