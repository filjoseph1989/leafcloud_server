...
# Must match the training script being evaluated — pass --v3 flag to evaluate V3 model
CONCENTRATION_MAP_V4 = {
    'Water': [0.0, 0.0],
    'NPK':   [1.0, 0.0],
    'Micro': [0.0, 1.0],
    'Mix':   [1.0, 1.0],
...
def get_validation_data(concentration_map):
    ...
    # Stratified split — mirrors V4 training split
...
def evaluate_classification(y_true_idx, clf_preds, export_dir, version_label):
    ...
    plt.title(f"Confusion Matrix - Nutrient Classification ({version_label})", fontsize=14, pad=15)
    plt.ylabel("Actual Label", fontsize=12)
    plt.xlabel("Predicted Label", fontsize=12)
    plt.tight_layout()

    os.makedirs(export_dir, exist_ok=True)
    save_path = os.path.join(export_dir, f"confusion_matrix_{version_label.lower()}.png")
    plt.savefig(save_path, dpi=300)
    plt.close()
    print(f"\n📊 Saved confusion matrix heatmap to: {save_path}")


def evaluate_regression(y_true_reg, reg_preds, synced_df, version_label):
    ...
    # Per-class regression breakdown — shows which classes drive regression error
    print("\nPer-Class Regression Breakdown:")
    print(f"  {'Class':<8}  {'Macro (pred avg)':>16}  {'Micro (pred avg)':>16}  {'Macro (true)':>12}  {'Micro (true)':>12}")
    for label in LABEL_LIST:
        ...
        print(f"  {label:<8}  {m_pred:>16.4f}  {u_pred:>16.4f}  {m_true:>12.4f}  {u_true:>12.4f}")

    # Output range check — confirms sigmoid is bounding outputs correctly
...
def main():
    ...
    use_v3_targets = '--v3' in sys.argv
    concentration_map = CONCENTRATION_MAP_V3 if use_v3_targets else CONCENTRATION_MAP_V4
    version_label = 'V3' if use_v3_targets else 'V4'

    # Accept model path as first non-flag arg
    args = [a for a in sys.argv[1:] if not a.startswith('--')]
    if args:
        model_path = args[0]
    else:
        import glob
        pattern = 'leafcloud_multimodal_v3_*.keras' if use_v3_targets else 'leafcloud_multimodal_v4_*.keras'
        keras_files = sorted(glob.glob(pattern), reverse=True)
        if not keras_files:
    ...
    print(f"💾 Loading model: {model_path} ({version_label} targets)...")
    model = tf.keras.models.load_model(model_path)
    post_load_ram = get_ram_usage_mb()

    val_df = get_validation_data(concentration_map)
    if val_df.empty:
    ...
    evaluate_classification(y_true_idx, clf_preds, export_dir='exports', version_label=version_label)
    evaluate_regression(y_true_reg, reg_preds, synced_df, version_label)
